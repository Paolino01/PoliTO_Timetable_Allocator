...
def add_no_overlap_constraint(model, timetable_matrix, teacher, slots):
    for s in slots:
        ...
        model.add(
            model.sum(timetable_matrix[t.id_teaching, s] for t in teacher.teachings if t.didactic_period[-1] == '1') <= 1
        )

        # Constraints for the second semester
        model.add(
            model.sum(timetable_matrix[t.id_teaching, s] for t in teacher.teachings if t.didactic_period[-1] == '2') <= 1
        )
