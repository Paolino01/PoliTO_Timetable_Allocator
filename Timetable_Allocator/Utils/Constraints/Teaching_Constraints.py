...
def add_teachings_constraints(model, timetable_matrix, teachings, slots, days):
    ...
    for t in teachings:
        ...
        # Adding constraint for laboratories
        # TODO: Needs to be implemented and tested with a Pro version of CPLEX
        '''
        if t.lab_hours != 0:
            model.add_constraint(model.sum(timetable_matrix[t.id_teaching + '_lab', s] for s in slots) == 1)
        '''
        ...
        for d in days:
            ...
            for s in range(len(slots) - 1):
                if math.floor(s / params.slot_per_day) == d and math.floor((s + 1) / params.slot_per_day) == d:
                    model.add(model.logical_or(timetable_matrix[t.id_teaching, s] == 0, (
                                timetable_matrix[t.id_teaching, s] + timetable_matrix[t.id_teaching, s + 1]) >= model.min(2, n_slots_in_day_teaching[t.id_teaching, d])))
